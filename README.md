...
| Technology | Purpose |
|---|---|
| **React 19** | Component-based JavaScript library for building clean, reactive user interfaces |
| **Vite 6** | Ultra-fast build tool and development server with hot module replacement (HMR) |
| **Tailwind CSS v4** | Modern utility-first CSS framework for modular styling and premium responsive layouts |
...
- **Session State Cache:** Persists form fields (`First Name`, `Last Name`, `Phone`, `Services`) in `sessionStorage` so inputs are not lost when navigating away or refreshing — a manual "Clear form" option is available
...
- **Responsive Canvas:** Floating network particles render dynamically on a `<canvas>` element — node count decreases from `30` (desktop) to `20` (mobile), and connection lines dynamically shorten on mobile screens
